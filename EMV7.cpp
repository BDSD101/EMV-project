...
/**
 * @brief  Drives a demonstration EMV payment session.
 *
 * Runs two transactions to show both authentication methods:
 *   Transaction 1 — Visa card, PASS authentication  (password123)
 *   Transaction 2 — Visa card, PIN authentication   (1234)
 *
 * The operator types the card number at the terminal prompt.
 * Demo card numbers are printed to the console for convenience.
 */
int main()
{
    try {
        ...
        std::cout << '\n' << SEP << '\n'
                  ...
                  << "  Demo card 1 (PASS): 4716893064521783  →  password: password123\n"
                  << "  Demo card 2 (PIN):  4929176890231561  →  PIN: 1234\n"
                  << SEP << '\n';

        // Transaction 1 — password-authenticated Visa card
        terminal.runTransaction(bank, 49.99, "Coffee Shop");

        // Transaction 2 — PIN-authenticated Visa card
        terminal.runTransaction(bank, 120.00, "Supermarket");

        // Print full transaction log
        terminal.displayTransactions();

        // Show RSA keys (demonstration only)
        std::cout << "\nBank RSA Keys (demonstration):\n" << SEP << '\n';
        bank.printKeys();
        std::cout << SEP << '\n';
